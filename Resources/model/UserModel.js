...
var encrypt = funtion(email, password, salt) {
...
/**
 * 
 * Gets a user if the user_id and callback exists
 * If they don't exist, we will just get the user from our local storage
 * 
...
UserModel.getUser = function(user_id, callback) {
	if (user_id && callback) {
		this.GET({ user_id : user_id }, function(result) {
			Ti.App.Properties.setObject(_word, result);
			callback(result);
		});
		return;
	}
